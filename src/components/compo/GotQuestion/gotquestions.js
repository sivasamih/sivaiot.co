"use client";
import { Card, Typography, Button, InputAdornment, } from "@mui/material";
...
export default function GotQuestion({ isOpen, setIsOpen }) {
  ...
  const handleOtpValidate = async () => {
    ...
    if (res.status === 200) {
      showSnackbar("Email verified successfully.Please Fill below details.", "success");
      setIsEmailValidate(true);
    ...
    <>
      ...
      <Card
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Typography>Ask your question and we will get back to you as soon as possible.</Typography>
          <Box sx={{ display: isMobileLandscape ? "grid" : "block", gridTemplateColumns: "1fr 1fr 1fr", columnGap: 1 }}>
            ...
            {!isOtpVisible && (
              <CustomTextField
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end" sx={{ pr: 0, mr: -1 }}>
                      {!IsEmailValidate && (
                        <Button size="small" variant="outlined" onClick={handleEmailVerify}>
                          Verify
                        </Button>
                      )}
                    </InputAdornment>
            ...
            {isOtpVisible && (
              <CustomTextField
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end" sx={{ pr: 0, mr: -1 }}>
                      <Button size="small" variant="outlined" onClick={handleOtpValidate}>
                        Validate
                      </Button>
                    </InputAdornment>
                  ),
            ...
            <CustomTextField
              ...
              disabled={!IsEmailValidate}
            />
            <Button variant="contained" fullWidth size="small" sx={{ mt: 1 }} onClick={handleSubmit} disabled={!IsEmailValidate}>
              Submit
            </Button>{" "}
            <Button variant="contained" fullWidth size="small" sx={{ mt: 1 }} onClick={handleReset}>
              Reset
            </Button>
          </Box>
        </Box>
